...
export function MouthShapeEditor({
  points,
  patch,
  preview,
  commitPreview,
  ...
  function updatePoint(pointIndex, point, notify) {
    ...
    notify({ customMouth: { points: nextPoints } });
  }
  ...
  function handleAddPoint(event) {
    ...
    patch({ customMouth: { points: insertPointOnSegment(points, clicked) } });
  }

  function handleRemovePoint(pointIndex) {
    if (points.length <= MIN_POINTS) return;
    patch({ customMouth: { points: removePointAt(points, pointIndex) } });
  }
  ...
  function handleScaleInput(event) {
    ...
    preview({
      customMouth: { points: scalePoints(scaleBaseRef.current, value / 100) },
    });
  }
  ...
  function applySavedShape(savedShape) {
    patch({ customMouth: { points: [...savedShape.points] } });
    setShapeName('');
  }

  function startNewShape() {
    patch({ customMouth: { points: [...DEFAULT_CUSTOM_MOUTH_POINTS] } });
    setShapeName('');
  }
  ...
  const coordinates = customMouthPointCoordinates(
    points,
    EDITOR_CENTER,
    EDITOR_CENTER,
  );
  const pathData = customMouthToPath(points, EDITOR_CENTER, EDITOR_CENTER);
  ...
  return (
    <div className="eye-shape-editor">
      <svg
        ...
        role="group"
        aria-label="Éditeur de bouche personnalisée"
      >
        <path
          d={pathData}
        ...
        {coordinates.map((point, index) => (
          <g key={index} className="shape-editor-point">
            <circle
              ...
              aria-label={`Point ${index + 1} de la bouche — glisser ou flèches pour déplacer`}
              cx={point.x}
              cy={point.y}
              r="4"
              {...dragHandlers(index)}
      ...
      <label className="thickness-control shape-editor-scale">
        ...
        <input
          ...
          max="200"
          value={scaleValue}
          aria-label="Échelle uniforme de la bouche"
            ...
            renamingId === savedShape.id ? (
              <div className="shape-editor-library-item" key={savedShape.id}>
                <div className="shape-editor-library-thumb shape-editor-library-renaming">
                  <svg viewBox="0 0 80 80" aria-hidden="true">
                    <path
                      d={customMouthToPath(savedShape.points, 40, 40)}
              ...
              <div className="shape-editor-library-item" key={savedShape.id}>
                <button
                  ...
                  aria-label={`Utiliser la bouche ${savedShape.name}`}
                ...
                <button
                  ...
                  aria-label={`Renommer la bouche ${savedShape.name}`}
                ...
                <button
                  ...
                  aria-label={`Supprimer la bouche ${savedShape.name}`}
